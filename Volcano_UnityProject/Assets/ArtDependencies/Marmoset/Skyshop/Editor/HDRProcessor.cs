...
namespace mset {
	...
	public class HDRProcessor : AssetPostprocessor {
		...
		private static List<int> _RGBMList = null;
		private static List<int> RGBMList {
			get {
				if(_RGBMList == null) _RGBMList = new List<int>();
				return _RGBMList;
			}
		}
		...
		public void OnPreprocessTexture() {
			//look for our texture in a list of recently loaded RGBM files (by hash to keep string compares to a minimum)
			int hash = this.assetPath.GetHashCode();
			//if this png is a recently-generated RGBM image, change its import settings
			if(mset.HDRProcessor.RGBMList.Contains(hash)) {
		...
		public static void OnPostprocessAllAssets ( string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths) {
			...
			for (int a=0; a<importedAssets.Length; ++a) {
				...
				if( readImage != null ) {
					...
					if(!RGBMList.Contains(hash)) RGBMList.Add(hash);
					...
					mset.AssetUtil.setReadable(srTex, true);
					if( readImage(ref tex, imagePath) ) {
						mset.AssetUtil.writePNG(ref tex, texPath);
